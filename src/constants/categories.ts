// src/constants/categories.ts — product categories and PayRex pharma flag
...
// ⚠️  FLIP TO false ONLY WHEN PAYREX APPROVES THE MERCHANT ACCOUNT.
export const HIDE_PHARMA_PUBLIC = true;

export const PUBLIC_CATEGORIES: Category[] = HIDE_PHARMA_PUBLIC
  ? CATEGORIES.filter(c => c.id !== "pharma")
  : CATEGORIES;

export const filterPharmaPublic = <T extends { category: string }>(arr: T[]): T[] =>
  HIDE_PHARMA_PUBLIC ? arr.filter(p => p.category !== "pharma") : arr;
